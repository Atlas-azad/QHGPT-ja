...
// =================================================================
// ==================== 老板，这是我为您添加的 "大脑记忆" ====================
// 它是一个“记忆保险箱”，只在当前页面会话中生效，刷新即忘，完美符合您的要求！
...
export default () => {
  ...
  const archiveCurrentMessage = () => {
    if (currentAssistantMessage()) {
      const newMsg = {
        role: 'assistant' as 'assistant', // 明确类型
  ...
  const clear = () => {
    ...
    // 清空缓存中当前角色的记录
    if (currentRole().role) {
      chatHistoryCache.delete(currentRole().role);
    }
  };
  ...
  // =================================================================
  // =========== 这是我为您重写的、最核心的 aichoiceRole 函数 ===========
  // 它现在拥有了完美的记忆和切换能力！
  const choiceRole = (newRole: Role) => {
    const oldRoleName = currentRole().role;
    const currentMessages = messageList();

    // 1. 保存：在切换走之前，把当前角色的聊天记录存进“保险箱”
    // 只有当旧角色存在且有聊天记录时才保存。
    if (oldRoleName && currentMessages.length > 0) {
      chatHistoryCache.set(oldRoleName, currentMessages);
    }
    
    // 2. 切换：更新为新的角色
    setCurrentRole(newRole);

    // 3. 加载：从“保险箱”里寻找新角色的历史记录
    const historyMessages = chatHistoryCache.get(newRole.role);

    if (historyMessages) {
      // 找到了！直接加载历史记录。
      setMessageList(historyMessages);
    } else {
      // 没找到（第一次和这个角色聊），则初始化欢迎语。
      setMessageList([]); // 先清空
      setCurrentAssistantMessage(newRole.fc);
      // 使用一个微任务来确保在下一次渲染中执行，避免状态冲突
  ...
  // Return的JSX部分保持不变，所以这里省略以节约篇幅。
  // 注意：您复制时需要包含下面的 return 部分直到文件结尾。
  return (
    <div my-6>
      <div>
        <div class="swiper">
