...
import IconMarkdown from './icons/Markdown'; // 虽然现在不用了，但为了未来可能用到的地方，先留着
...
export default (props: GeneratorProps) => {
  ...
  const [roles, setRoles] = createSignal<Role[]>(props.roles);
  ...
  onMount(async () => {
    try {
      ...
      if (storedSetting) setSetting({ ...defaultSetting, ...JSON.parse(storedSetting) });
    } catch { console.error("Setting parse error"); }

    try {
      const storedHistory = localStorage.getItem(CHAT_HISTORY_KEY);
      if (storedHistory) setChatHistory(JSON.parse(storedHistory));
    } catch { console.error("History parse error"); }
    
    const loadedRoles = roles();
    if (loadedRoles.length > 0) {
      const lastRoleName = localStorage.getItem('last_selected_role');
      const lastRole = lastRoleName ? loadedRoles.find(r => r.role === lastRoleName) : null;
      choiceRole(lastRole || loadedRoles[0]);
    }
  });
  ...
  const modifyHistoryForCurrentRole = (updateFn: (prevMessages: ChatMessage[]) => ChatMessage[]) => {
    const roleName = currentRole()?.role;
    if (!roleName) return;
    
    setChatHistory(prevHistory => ({
      ...prevHistory,
      [roleName]: updateFn(prevHistory[roleName] || [])
    }));
  };
  ...
  const requestWithLatestMessage = async () => {
    ...
    try {
      ...
      requestMessageList = requestMessageList.filter(item => !item.content.includes('⚠️') && !item.content.includes('网络似乎有些波动'));
      ...
      while (!done) {
        ...
        if (value) {
          ...
          if (char) setCurrentAssistantMessage(current => current + char);
        }
        done = readerDone;
      }
    } catch (e: any) {
      if (e.name === 'AbortError') {
        ...
        setCurrentAssistantMessage(`⚠️AI服务商响应错误: ${e.message}`);
      }
    } finally {
  ...
  const archiveCurrentMessage = () => {
    ...
    if (msg) {
      modifyHistoryForCurrentRole(prev => [...prev, { role: 'assistant', content: msg }]);
    }
    setCurrentAssistantMessage('');
    setLoading(false);
    setController(null);
  ...
  const choiceRole = (newRole: Role | null) => {
    if (!newRole) return;
    ...
    localStorage.setItem('last_selected_role', newRole.role);
    if(inputRef) inputRef.focus();
  };

  const retryLastFetch = () => {
    modifyHistoryForCurrentRole(prev => {
        const lastUserMessageIndex = _.findLastIndex(prev, { role: 'user' });
        if (lastUserMessageIndex === -1) return prev;
        
        const messagesForRetry = prev.slice(0, lastUserMessageIndex + 1);
        requestWithLatestMessage();
        return messagesForRetry;
    });
  };
  ...
  const exportToMarkdown = () => {
    ...
    link.download = `${roleName}-对话记录.md`;
    ...
    <div my-6>
      <div>
        <div class="swiper">
          <div class="swiper-wrapper">
            <For each={roles()} fallback={<div class="p-4 text-center text-slate-500">正在召唤诸位圣贤...</div>}>
              {(item) => (
                <div
      ...
      >
        <div class="my-4 flex items-center gap-2 transition-opacity">
          ...
          {/* 【V6.1 细节修正】: 老板，您的宝印已归位！我保留了我统一的按钮样式，但将内部图标完美还原！ */}
          <button title="导出Markdown" onClick={exportToMarkdown} class="h-12 px-2 py-2 bg-slate bg-op-15 rounded-lg hover:bg-slate-50 transition-all duration-200">
