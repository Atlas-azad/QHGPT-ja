...
export default () => {
  ...
  // BOSS: 新增一个 state，用于存放从 localStorage 加载的所有角色的聊天记录。
  ...
  onMount(async () => {
    ...
    try {
      if (storage) {
        ...
        setSetting({ ...defaultSetting, ...parsed });
      }
    } catch {
      console.log("Setting parse error");
    }
    
    // BOSS: 重构了这里的逻辑，用于加载所有角色的聊天记录。
    // 1. 从 localStorage 读取我们统一存放的聊天记录
    ...
    if (historyStorage) {
      try {
        ...
        console.log("Chat history parse error");
      }
    }
    ...
    // 3. 自动选择第一个角色，并加载它的历史记录
    if (fetchedRoles.length > 0) {
      const firstRole = fetchedRoles[0];
      setCurrentRole(firstRole);
      const roleHistory = history[firstRole.role] || [];
      
      // 如果没有历史记录，就显示角色的第一句话
      if (roleHistory.length === 0 && firstRole.fc) {
        setMessageList([{ role: 'assistant', content: firstRole.fc }]);
      } else {
  ...
  // BOSS: 删除了这里原来有缺陷的保存逻辑。

  // BOSS: 这是全新的、健壮的聊天记录保存逻辑。
  // 当 messageList 或 currentRole 变化时，它会自动保存。
  createEffect(() => {
    const roleId = currentRole().role;
    if (!roleId) return; // 如果还没有角色，则不保存

    // 更新 allHistory state 中当前角色的记录
    setAllHistory(prev => ({
      ...prev,
      [roleId]: messageList()
    }));

    // 将整个 allHistory 对象保存到 localStorage
    localStorage.setItem(CHAT_HISTORY_STORAGE_KEY, JSON.stringify(allHistory()));
  });
  ...
  const handleButtonClick = async () => {
    ...
    // BOSS: 优化了这里的逻辑，如果当前只有一句开场白，就替换它，否则追加。
    ...
    if (isInitialMessage) {
        setMessageList([currentList[0], newUserMessage]);
    } else {
  ...
  const requestWithLatestMessage = async () => {
    ...
    try {
      ...
      // BOSS: 我们保留第一条 assistant 的消息（例如开场白），API 请求时再去掉。这可以防止开场白在UI中消失。
      if (requestMessageList[0].role == 'assistant') {
        const { role, prompt } = roles().find(r => r.role === currentRole().role) || {};
        if (prompt) {
          // 在发给API的消息列表中，加入system prompt
          requestMessageList.unshift({role: 'system', content: prompt});
        }
      }
  ...
  // BOSS: 这里的 clear 函数现在只清空当前角色的聊天记录。
  const clear = () => {
    inputRef.value = '';
    inputRef.style.height = 'auto';
    // 只清空当前对话列表，上面的 createEffect 会自动保存这个空列表到 localStorage
    setMessageList([]); 
    setCurrentAssistantMessage('');

    // BOSS: 清空后，重新显示当前角色的开场白，体验更好。
  ...
  // BOSS: 这是最核心的修改！重写了 choiceRole 函数！
  const choiceRole = (e: Role) => {
    archiveCurrentMessage(); // 先把当前可能正在输入的AI消息归档
    stopStreamFetch();     // 停止任何正在进行的消息流
    
    if (currentRole().role === e.role) return; // 如果点击的是当前角色，则不操作

    setCurrentRole(e);
    
    // 从 allHistory state 中加载新选定角色的历史记录
    const history = allHistory()[e.role] || [];
    
    // 如果该角色没有任何历史记录，则显示它的开场白
    if (history.length === 0 && e.fc) {
      setMessageList([{ role: 'assistant', content: e.fc }]);
    } else {
  ...
  // ... JSX 部分保持不变，无需修改 ...
  // [为了简洁，我省略了 return (...) 的 JSX 部分，因为那里不需要改动]
  return (
    <div my-6>
      {/* ...Swiper, Message List, Input bar 等 JSX... */}
      {/* ...和您给我的原文件一样即可... */}
       <div>
        <div class="swiper">
